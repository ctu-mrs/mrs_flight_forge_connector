...
bool getCoordinates(std::string choice,
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    std::cout << "Usage: debug-cli <port> [address]" << std::endl;
    std::cout << "  port    - drone TCP port (e.g. 4000)" << std::endl;
    std::cout << "  address - simulator host IP (default: " << LOCALHOST << ")"
              << std::endl;
    return 1;
  }

  int port = atoi(argv[1]);
  std::string address = (argc >= 3) ? argv[2] : LOCALHOST;
  std::cout << "Will connect to " << address << ":" << port << std::endl;
  ...
  UedsConnector =
      std::make_unique<ueds_connector::UedsConnector>(address, port);
  auto connect_result = UedsConnector->Connect();
  if (connect_result != 1) {
    std::cout << "Error connecting to drone controller. connect_result was "
              << connect_result << std::endl;
    return 1;
  }
  ...
  while (true) {
    ...
    std::cout << "Set location and rotation: 6 (fixed 500 500 10000 10 20 30)"
              << std::endl;
    ...
    if (choice_char == 'x') {
      ...
      if (parseRes) {
        const auto [res, teleportedTo, isHit, impactPoint] =
            UedsConnector->SetLocation(coordinates, true);
        if (res) {
          std::cout << "SetLocation successful: teleported to: ("
                    << teleportedTo.x << ", " << teleportedTo.y << ", "
                    << teleportedTo.z << "), isHit: " << isHit
                    << ", impactPoint: (" << impactPoint.x << ", "
                    << impactPoint.y << ", " << impactPoint.z << ")"
                    << std::endl;
        } else {
      ...
      const auto [res, camera_data, stamp, size] =
          UedsConnector->GetRgbCameraData();
      if (res) {
        std::cout << "camera_data(ms): "
                  << std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::steady_clock::now() - start)
                         .count()
                  << std::endl;
        std::cout << "GetCameraData successful. Size: " << size << std::endl;
        std::cout << "Elapsed to get img (ms): "
                  << std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::steady_clock::now() - start)
                         .count()
                  << std::endl;
        ...
        //        file.write(reinterpret_cast<const char*>(camera_data.get()),
        //        size);
        copy(camera_data.cbegin(), camera_data.cend(),
      ...
      if (!res) {
        ...
        std::cout << "Rotation: "
                  ...
                  << " roll: " << rotation.roll << std::endl;
      }
    } else if (choice_char == '5') {
      ueds_connector::Rotation rotation{};
      auto parseRes = getRotation(choice, rotation);
      if (parseRes) {
        const auto [res, rotatedTo, isHit, impactPoint] =
            UedsConnector->SetRotation(rotation);
        if (res) {
          std::cout << "SetRotation successful: rotatedTo"
                    << rotatedTo.toString() << ", isHit: " << isHit
                    << ", impactPoint: " << impactPoint.toString() << std::endl;
        } else {
      ...
      const auto [res, teleportedTo, rotatedTo, isHit, impactPoint] =
          UedsConnector->SetLocationAndRotation(location, rotation, false);
      if (res) {
        std::cout << "SetLocationAndRotation successful: teleported to: "
                  << teleportedTo.toString()
                  << ", rotatedTo: " << rotatedTo.toString()
                  << ", isHit: " << isHit
                  << ", impactPoint: " << impactPoint.toString() << std::endl;
      } else {
      ...
      if (res) {
        std::cout << "GetLidarConfig successful: (Enable: " << config.Enable
                  ...
                  << ", BeamHorRays: " << config.BeamHorRays
                  << ", BeamVertRays: " << config.BeamVertRays
                  << ", Frequency: " << config.Frequency
                  << ", offset: " << config.offset.toString()
                  << ", rotation: " << config.orientation.toString()
                  << ", FOVHorLeft: " << config.FOVHorLeft
                  << ", FOVHorLeft: " << config.FOVHorLeft
                  << ", FOVVerUp: " << config.FOVVertUp
                  << ", FOVVerDown: " << config.FOVVertDown << ")" << std::endl;
      } else {
      ...
      if (res) {
        std::cout << "GetCameraConfig successful: (showDebugCamera: "
                  << config.show_debug_camera_
                  << ", offset: " << config.offset_.toString()
                  << ", rotation: " << config.orientation_.toString()
                  << ", Width: " << config.width_
                  << ", Height: " << config.height_ << ")" << std::endl;
      } else {
      ...
      if (res) {
        std::cout << "camera_data(ms): "
                  << std::chrono::duration_cast<std::chrono::milliseconds>(
                         ...
                         .count()
                  << std::endl;
        std::cout << "GetRangefinderData successful. Range:  "
                  << range_data / 100.0 << " [m]" << std::endl;
        std::cout << "Elapsed to get img (ms): "
                  << std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::steady_clock::now() - start)
                         .count()
      ...
      if (res) {
        ...
        if (size != width * height * 4) {
          std::cerr << "Size error: expected " << (width * height * 4)
                    << ", got " << size << std::endl;
          continue;
        }
        ...
        const uint8_t *bgra_data =
            camera_data.data(); // Cast for easier indexing
        ...
        if (fpng::fpng_encode_image_to_memory(rgb_data.data(), width, height, 3,
                                              png_data)) {
